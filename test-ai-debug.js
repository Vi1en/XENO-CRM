// Test the exact same logic as in the backend
function generateSmartMockRules(prompt) {
  ...
  console.log('=== DEBUGGING AI SEGMENT GENERATION ===');
  console.log('Input prompt:', prompt);
  console.log('Lower prompt:', lowerPrompt);
  
  // Check for spending-related keywords
  const hasSpendingKeywords = lowerPrompt.includes('spent') || lowerPrompt.includes('spend') || lowerPrompt.includes('purchase');
  console.log('Has spending keywords:', hasSpendingKeywords);
  
  if (hasSpendingKeywords) {
    console.log('Processing spending keywords...');
  ...
  const hasVisitKeywords = lowerPrompt.includes('visit') || lowerPrompt.includes('visited') || lowerPrompt.includes('times');
  console.log('Has visit keywords:', hasVisitKeywords);
  
  if (hasVisitKeywords) {
    console.log('Processing visit keywords...');
    ...
    console.log('Detected visits:', visits);
  ...
  const hasTimeKeywords = lowerPrompt.includes('last 30 days') || lowerPrompt.includes('30 days') || lowerPrompt.includes('recent');
  console.log('Has time keywords:', hasTimeKeywords);
  
  if (hasTimeKeywords) {
    console.log('Processing time keywords...');
  ...
  const hasTagKeywords = lowerPrompt.includes('vip') || lowerPrompt.includes('premium') || lowerPrompt.includes('loyal');
  console.log('Has tag keywords:', hasTagKeywords);
  
  if (hasTagKeywords) {
    console.log('Processing tag keywords...');
    rules.push({
      field: 'tags',
      operator: 'contains',
      value: 'vip'
  ...
  console.log('Final rules count:', rules.length);
  console.log('Final name:', name);
  console.log('Final description:', description);
  
  // If no specific rules were generated, return default rules
  if (rules.length === 0) {
    console.log('No rules generated, returning default');
    return {
      rules: [
        {
          field: 'totalSpend',
...
// Test with the exact prompt that's failing
const testPrompt = "Customers who spent more than $1000";
console.log('\n🧪 Testing with prompt:', testPrompt);
const result = generateSmartMockRules(testPrompt);
