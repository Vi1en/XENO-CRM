...
interface ApiDocsSafeProps {
  className?: string
}

export default function ApiDocsSafe({ className = '' }: ApiDocsSafeProps) {
  const { token } = useAuth()
  ...
  // Ensure we're on the client side and load SwaggerUI dynamically
  useEffect(() => {
    setIsClient(true)
    ...
    const loadSwaggerUI = async () => {
      try {
        ...
        // Load CSS separately to avoid TypeScript issues
  ...
  // Test connection to swagger.json
  useEffect(() => {
    if (!isClient) return
  ...
  if (!SwaggerUI) {
    return (
      <div className={`flex items-center justify-center min-h-[400px] ${className}`}>
        <div className="text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-yellow-600 to-orange-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce-gentle shadow-lg">
            <span className="text-white font-bold text-2xl">⏳</span>
          </div>
          <p className="text-gray-600 animate-pulse font-medium">Preparing API Documentation...</p>
          <p className="text-sm text-gray-500 mt-2">Loading Swagger UI components</p>
        </div>
      </div>
    )
  }
